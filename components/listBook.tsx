...
export function ListBook() {
  ...
  return (
    <div className={`w-5/6 h-[90%] relative top-8 border border-red-600 bg-black bg-opacity-50 rounded-2xl flex flex-col items-center`} onClick={() => onLeave()}>
      ...
      <div
        className={`w-[95%] h-[78%] flex flex-wrap ${
          onLoading ? " items-center justify-center" : "justify-start"
        }   border-2 border-red-400/60 rounded-xl bg-black/70 overflow-y-auto absolute top-[165px] scrollbar-hide pl-10 pt-6`}
      >
        {onLoading ? (
          <>
...
function SearchBar({ className }: any) {
  ...
  return (
    <>
      <div className={`${className} w-full h-[5%] flex justify-center relative gap-3`}>
