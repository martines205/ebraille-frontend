...
type ADD_BOOK_DATA_CONTEXT_TYPE = [FORM_INPUT_FILED, React.Dispatch<React.SetStateAction<FORM_INPUT_FILED>>];

const AddBookDataContext = createContext<ADD_BOOK_DATA_CONTEXT_TYPE>([{ AUTHOR: "", CATEGORY: "", EDITION: "", LANGUAGE: "", PUBLISHER: "", TITLE: "", YEAR: "", UPLOADER: "" }, () => null]);
...
export function AddBook({ role }: addBookInterface) {
  ...
  return (
    <>
      <AddBookDataContext.Provider value={[bookData, setBookData]}>
      ...
      </AddBookDataContext.Provider>
    </>
  );
}

export function BookPreview() {
  const [bookData, _] = useContext(AddBookDataContext);
  return (
    <div className="w-[80%] h-[90%] bg-black/80 rounded-lg border-2 border-red-500 relative flex items-center flex-col pt-9">
      <div className="h-80 w-60 border-[3px] border-red-600/60 rounded-r-md rounded-l-3xl bg-blue-700 relative flex">
        ...
        <div className="h-full w-full bg-red-500/0 flex items-center flex-col gap-3 pt-8">
          ...
          <h1 className=" text-black text-lg">{bookData.TITLE === "" ? "-" : bookData.TITLE}</h1>
...
export function FormAdd() {
  const [bookData, setBookData] = useContext(AddBookDataContext);
  ...
  return (
    <form
      className="bg-black/50 w-[70%] h-[90%] border-2 border-red-500 rounded-xl relative"
